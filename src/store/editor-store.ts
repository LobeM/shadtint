  ...
  persist(
    (set, get) => ({
      ...
      setThemeState: (newState: ThemeEditorState) => {
        ...
        // Check if only currentMode has changed
        ...
        const lastHistoricEntry =
          currentHistory.length > 0 ? currentHistory[currentHistory.length - 1] : null;

        if (
          !lastHistoricEntry ||
          currentTime - lastHistoricEntry.timestamp > HISTORY_OVERRIDE_THRESHOLD_MS
        ) {
      ...
      hasThemeChangedFromCheckpoint: () => {
        const checkpoint = get().themeCheckpoint;
        return !isDeepEqual(checkpoint, get().themeState);
      },
      hasUnsavedChanges: () => {
      ...
      resetToCurrentPreset: () => {
        ...
        set({ themeState: newThemeState, themeCheckpoint: newThemeState, history: [], future: [] });
      },
      undo: () => {
        ...
        const newFutureEntry: ThemeHistoryEntry = {
          state: currentThemeState,
          timestamp: Date.now(),
        };
        const newFuture = [...future, newFutureEntry];
