...
// Mut be in Sync with the Built-in Fonts
...
export function buildFontFamilyValue(font: string, category: FontCategory): string {
  return `${font}, ${SYSTEM_FONTS_FALLBACKS[category]}`;
}
...
export function extractFontFamily(fontFamilyValue: string): string | null {
  ...
  // Skip system fonts
  if (SYSTEM_FONTS.includes(cleanFont)) return null;
  return cleanFont;
}
...
export function getDefaultWeights(variants: string[]): string[] {
  ...
  // If none of the preferred weights are available, use the first two available weights
  ...
  const finalWeights = [
    ...selectedWeights,
    ...availableWeights.filter((weight) => !selectedWeights.includes(weight)),
  ].slice(0, 4);
